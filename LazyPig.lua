BINDING_HEADER_LP_HEADER = "LazyPig";
...
BINDING_NAME_WSGDROP = "Drop WSG Flag/Remove Noggen Light";
BINDING_NAME_MENU = "LazyPig Menu";
...
local LazyPigMenuStrings = {
		...
		[52]= "QUEUE BG",
		[60]= "IMMER",
		[61]= "KRIEGER SCHILD/DRUIDE BÄR",
		[90]= "SUMMON AUTO AKZEPTIEREN",
		[91]= "LOOT WINDOW AUTO POSITION",
		[92]= "VERBESSERTER RECHTS KLICK",
		[93]= "EASY SPLIT/MERGE(SHIFT+RECHTS KLICK)",
		[94]= "EXTENDED CAMERA DISTANCE",
		[95]= "SPECIAL KEY COMBINATIONS",
		[96]= "DUEL AUTO DECLINE",
		[97]= "RESURRECTION AUTO ACCEPT OOC",
...
LPCONFIG = {DISMOUNT = true, CAM = false, GINV = true, FINV = true, SINV = nil, SUMM = true, EBG = true, LBG = true, QBG = false, LOOT = true, EPLATE = false, FPLATE = false, HPLATE = false, RIGHT = true, ZG = 1, DUEL = false, NOSAVE = false, GREEN = 2, SPECIALKEY = true, WORLDDUNGEON = false, WORLDRAID = false, WORLDBG = false, WORLDUNCHECK = nil, SPAM = false, SHIFTSPLIT = true, REZ = true, GOSSIP = true, SALVA = false}
				...
				for i=1,STATICPOPUP_NUMDIALOGS do
					...
					if frame:IsShown() then
						...
						if frame.which == "DEATH" and HasSoulstone() then
							...
							if passpopup < current_time then delayaction = current_time + 0.75 end
...
function LazyPig_OnEvent(event)	  
	if (event == "PLAYER_ENTERING_WORLD") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("LazyPig by Ogrisch loaded type".."|cff00ff00".." /lp".."|cffffffff or click "..LazyPig_CreateLink("Hyperlink", 1, GREEN).." for options")
	...
	elseif(event == "DUEL_REQUESTED") then
		duel_active = true
		if LPCONFIG.DUEL then --dnd_active and
			duel_active = nil
			CancelDuel()
	...
	elseif(event == "RESURRECT_REQUEST" and LPCONFIG.REZ) then
		...
		if GetCorpseRecoveryDelay() == 0 and UnitIsVisible("target") and not UnitAffectingCombat("target") then
			AcceptResurrect()
...
function MailtoCheck(msg) 
	if MailTo_Option then -- to avoid conflicts with mailto addon
		...
		if msg then DEFAULT_CHAT_FRAME:AddMessage("LazyPig: Warning Improved Right Click and Easy Split/Merge features may override MailTo addon functionality !") end
	end
end
...
function LazyPig_ZGRoll(id)
	...
	if LPCONFIG.ZG then	
		...
		if string.find(name ,"Schmuckstück") or string.find(name ,"Münze") or string.find(name ,"skarabäus") or string.find(name ,"götze") or string.find(name ,"Götze") then
...
function LazyPig_Dismount()
	local counter = 0
	local tooltipfind = "Erhöht Tempo um (.+)%%"
...
function LazyPig_BG()
	local t = GetRealZoneText()
	if t == "Arathi Becken" or t == "Warsong Schlucht" or t == "Alterac Tal" then
		return true
	end
	return false
end
		...
		if ChatFrame:IsVisible() and not UnitIsDeadOrGhost("player") then
			...
			if (LPCONFIG.WORLDRAID or LPCONFIG.WORLDDUNGEON or LPCONFIG.WORLDBG) and not leavechat then						
				if id == 0 then
					...
					if not process(ChatFrame, name) or channelstatus then
...
function LazyPig_IsBearForm()
	...
	for i = 1 , max do
		...
		if(isActive and LazyPig_PlayerClass("Druid", "player") and (name == "Bärengestalt" or name == "Terrorbärengestalt")) then
			return true
		end
	end
	return false
		...
		if untilCancelled ~= 1 then
			...
			while buff[i] do
				if string.find(GetPlayerBuffTexture(index), buff[i]) then
					...
					UIErrorsFrame:Clear();
					UIErrorsFrame:AddMessage("Segen der Rettung wurde entfernt.");
					return
				end
				i = i + 1
			end	
...
function LazyPig_ChatFrame_OnEvent(event)
	if LPCONFIG.SPAM and arg2 and arg2 ~= GetUnitName("player") and (event == "CHAT_MSG_SAY" or event == "CHAT_MSG_CHANNEL" or event == "CHAT_MSG_YELL") then
		local time = GetTime()
		local pass = true
		local f, c = ChatMessage["FL"], ChatMessage["CL"]
		for blockindex,blockmatch in pairs(ChatMessage[f]) do
			if blockmatch == time and blockindex == arg1 then
				return
			elseif (blockmatch + 60) > time then --60s delay
				if blockindex == arg1 then
					ChatMessage[c][blockindex] = ChatMessage[f][blockindex]
					pass = nil
				end
			end
		end
		if pass then
			ChatMessage[f][arg1] = time
		else
			ChatMessage[f] = {}
			ChatMessage["FL"], ChatMessage["CL"] = c, f
			return
		end	
	end	
